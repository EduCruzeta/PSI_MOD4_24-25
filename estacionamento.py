...
def LerDados(matriculas,tempos):
    ...
    # verificar se as listas estam com o mesmo tamanho
    if len(matriculas_separadas) != len(tempos_separados):
        print("O número de matriculas deve ser igual ao número de tempos")
        return
...
def MaiorTempoDeEstacionamento(matriculas,tempo):
    ...
    for p in MAX_LUGARES:
        ...
        if tempos[p] > tempos[p_maior]:
            p_maior = p
    print(f"O tempo maior de estacionamento foi de {tempos[p_maior]} e corresponde a matricula {matriculas[p_maior]}")
...
def ListaSuperiorMedia(matriculas,tempos):
    ...
    for p in range(MAX_LUGARES):
        if tempos[p] == 0:
            break
        if tempos[p] > media:
            print(f"{matriculas[p]} esteve {tempos[p]} que é superior à média")
    
# Verificar se existe algum carro que esteve no estacionamneto mais que uma vez
def Listar_repetidos(matriculas):
    for m in matriculas:
        contar = 0
        if m =="":
            break
...
# Permitir a pesquisa de uma matricula inserida pelo utilizador e mostar os tempos dessa matrticula casp existam


# Mostar a lista de matriculas ordenadas pelos tempos de estacionamento(maior para o menor)
